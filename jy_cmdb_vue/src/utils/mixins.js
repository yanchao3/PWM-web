...
export var list_mixin = {
    data() {
        return {
            ...
            pageLen: 5,
            params: {
                page_size: Math.floor((window.innerHeight - 340) / 80) < 7 ? 7 : Math.floor((window.innerHeight - 340) / 80), //每页数据个占屏幕的80%
                page: 1,
                ordering: "",
                search: "",
                token: "",
    ...
    methods: {
        get_data(reset_page = false,filter1='') {
            if (reset_page) {
                this.params.page = 1
            }
            this.loading = true
            ...
            console.log("gggggggggggg",this.$store.state.token);
            this.params.search = filter1;
            this.$c_master.get(this.url, { params: this.params }).then(response => {
                console.log("adddddddd",response);
                this.loading = false
        ...
        on_search() {
            console.log("sssssseeeeee");
            this.get_data(reset_page=true);
        },
        add_dialog(comp_name, props = {}) {
            console.log("ccccccccccc",comp_name,props)
            console.log("tttttttttt",this,this.url)
            // 设定dialog提交的时候 请求的路径
            props["p_parent"] = this
...
export const dialog_mixin = {
    ...
    data() {
        return {
            ...
            url: this.p_type === 'update' ? `${this.p_parent.url}/${this.p_item ? this.p_item.id : this.p_item_id}`: this.p_parent.url,
            loading: false,
            data: this.p_is_update ? this.p_is_update : {},
            form_data: this.p_type === "update" ? Object.assign({}, this.p_item) : (this.p_type === "clone" ? Object.assign({}, this.p_item) : {
              monitor_mode:[],
    ...
    methods: {
        on_submit(emit_func) {
            if (["add_item", "update_item", "delete_item", null].indexOf(emit_func) === -1) {
                emit_func = this.p_type === "create" ? "add_item" :(this.p_type === "clone" ? "create" : "update_item" )
            }
            this.submit_loading = true
            console.log("ffffff",this.form_data)
...
export const dialog_mixin2 = {
    ...
    methods: {
        on_submit(emit_func) {
            console.log("rrrrrrrrrrrrrrr",this.p_item);
            ...
            this.submit_loading = true
            console.log("llllllllllll",this.url)
            this.$c_master.request({
                url: this.p_type === 'action' ? `${this.url}/${this.action_url}` : this.url,
                method: this.p_type === 'update' ? "put" : "post",
                data: this.form_data
