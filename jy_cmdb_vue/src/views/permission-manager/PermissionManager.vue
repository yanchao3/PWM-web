<template>
  <div>
    <c-option-bar>
      <el-button size="medium" type="primary" @click="add_dialog('AppPermissionmanagerCreateUpdate', {p_type: 'create'})" icon="el-icon-plus">添加</el-button>
      <c-search placeholder="用户组" :parent="this"></c-search>
    </c-option-bar>
    ...
    <el-table :data="data" border stripe size="medium" @filter-change="on_change_filter" @sort-change="on_change_sort" v-loading="loading" style="width: 100%" :row-style="getRowClass" :header-row-style="getRowClass" :header-cell-style="getRowClass">
      ...
      <el-table-column label="标题" prop="alias" show-overflow-tooltip>
      </el-table-column>
      <el-table-column label="前端路由名称" prop="name" show-overflow-tooltip>
      </el-table-column>
      <el-table-column label="所属菜单" prop="menu_info" show-overflow-tooltip>
      </el-table-column>
      <el-table-column label="操作" width="300">
        <template slot-scope="scope">
          <el-button size="mini" type="warning" @click="add_dialog('AppPermissionmanagerCreateUpdate', {p_type: 'update', p_item: scope.row})">修改</el-button>
