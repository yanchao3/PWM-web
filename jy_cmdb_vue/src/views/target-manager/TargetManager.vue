<template>
  <div>
    <c-option-bar>
      <el-button size="medium" type="primary" @click="add_dialog('AppTargetCreateUpdate', {p_type: 'create'})" icon="el-icon-plus">添加</el-button>
      <c-search placeholder="ID/target名称" :parent="this"></c-search>
    </c-option-bar>
    <el-table :data="data" border stripe size="medium" @filter-change="on_change_filter" @sort-change="on_change_sort" v-loading="loading" style="width: 100%" :row-style="getRowClass" :header-row-style="getRowClass" :header-cell-style="getRowClass">
      ...
      <el-table-column label="target名称" prop="target_name" show-overflow-tooltip>
      </el-table-column>
      <el-table-column label="IP" prop="address" show-overflow-tooltip>
      </el-table-column>
      <el-table-column label="端口" prop="port" show-overflow-tooltip>
      </el-table-column>
      <el-table-column label="所属JOB" prop="job_name" show-overflow-tooltip>
      </el-table-column>
      <el-table-column label="操作" width="230">
        <template slot-scope="scope">
          <el-button size="mini" type="warning" @click="add_dialog('AppTargetCreateUpdate', {p_type: 'update', p_item: scope.row})">修改</el-button>
